...
final visualizerBandsProvider = StreamProvider<List<double>>((ref) {
  ...
  // frame timing). New prefixes miss the old 12-float cache files, which
  // age out of the extractor's LRU.
  ...
  void startExtraction(Track track, String key) {
    final path = track.filePath;
    if (path != null) {
      ...
      resolver.decodableStreamUrl(track).then((url) {
        // Only fire if this key is still wanted (primary or prewarm).
        if (url != null && (key == currentKey || key == prewarmKey)) {
          FftChannel.start(url, key);
        }
      });
    }
  }
  ...
  /// done or was never decodable (throttled stream = nothing to lose).
